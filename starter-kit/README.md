...
**規則層｜寫給 AI 看的判斷力外化（`00_專案控制/`）**：`about-me.md`（你是誰、目標與優先序，含寫作慣性自我申報）、`research-identity.md`（研究定位與理論版圖）、`writing-standards.md`（寫作規範與個人風格庫）、`portfolio-status.md`（發表組合動態儀表板）。四檔皆為欄位模板，內含填寫指引與虛構範例句，使用前請完整填寫，並用你自己的話寫；填寫後任務執行時視為唯讀，僅在你主動更新進度時修改。
...
**治理基座｜資料夾讀寫規則**：`01_文獻/`、`02_資料與證據/`、`03_寫作/`、`04_審稿與回應/`、`05_輸出/` 五夾對應研究流程的不同階段。00–04 五夾在任務執行時唯讀，AI 只讀取不寫入；`05_輸出/` 是所有 AI 產出的唯一寫入區，依專案建子資料夾分流。各夾詳細讀寫紀律與檔名規則見各自的 `README.md`。
